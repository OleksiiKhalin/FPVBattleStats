...
type Props = {
  ...
  onOpponentChange?: (pilot: string) => void;
};

type ComparisonView = "calendar" | "gap-chart";

const RECENT_PILOTS_KEY = "fpvbattle-recent-comparison-pilots";
const DEFAULT_OPPONENT = "Jaxon";
...
function rollingAverage(values: Array<number | null>, windowSize: number, excludeWorst: boolean) {
...
export function PilotComparisonPanel({ primaryPilot, raceClass, dateFrom, dateTo, initialOpponent, onOpponentChange }: Props) {
  const [opponentPilot, setOpponentPilot] = useState(initialOpponent ?? DEFAULT_OPPONENT);
  ...
  const [comparisonView, setComparisonView] = useState<ComparisonView>("calendar");
  const [showDailyGap, setShowDailyGap] = useState(true);
  const [showWeeklyAverage, setShowWeeklyAverage] = useState(false);
  const [showMonthlyAverage, setShowMonthlyAverage] = useState(false);
  const [excludeWorstResult, setExcludeWorstResult] = useState(false);
  ...
  useEffect(() => {
    if (opponentPilot) onOpponentChange?.(opponentPilot);
  }, [onOpponentChange, opponentPilot]);
  ...
  const selectOpponent = (pilot: string) => {
    ...
    sessionStorage.setItem("fpvbattle-comparison-opponent", pilot);
  };
  ...
  const gapChartData = useMemo(() => {
    ...
    const primaryValues = days.map((day) => day.primary_gap_to_leader_percentage);
    const opponentValues = days.map((day) => day.opponent_gap_to_leader_percentage);
    const primaryWeekly = rollingAverage(primaryValues, 7, excludeWorstResult);
    const opponentWeekly = rollingAverage(opponentValues, 7, excludeWorstResult);
    const primaryMonthly = rollingAverage(primaryValues, 30, excludeWorstResult);
    const opponentMonthly = rollingAverage(opponentValues, 30, excludeWorstResult);
    return days.map((day, index) => ({
      date: day.date,
      primary_daily_gap: primaryValues[index],
      opponent_daily_gap: opponentValues[index],
      primary_weekly_gap: primaryWeekly[index],
      opponent_weekly_gap: opponentWeekly[index],
      primary_monthly_gap: primaryMonthly[index],
      opponent_monthly_gap: opponentMonthly[index],
    }));
  }, [comparison.data, excludeWorstResult]);

  const comparisonReturnPath = useMemo(() => {
    const params = new URLSearchParams({ class: raceClass, pilot: primaryPilot, section: "compare", opponent: opponentPilot });
    if (dateFrom) params.set("from", dateFrom);
    if (dateTo) params.set("to", dateTo);
    return `/pilot-stats?${params.toString()}`;
  }, [dateFrom, dateTo, opponentPilot, primaryPilot, raceClass]);
  ...
  return (
    <div className="stack">
      <section className={open ? "panel comparison-panel-with-open-menu" : "panel"}>
        <div className="panel-header">
          <div><p className="eyebrow">Head-to-head</p><h2>Compare {primaryPilot} against another pilot</h2></div>
          <div className="meta"><span>Statistics use the selected class and date scope.</span></div>
        </div>
      ...
      {comparison.data ? <>
        ...
        <section className="panel">
          <div className="panel-header"><div><p className="eyebrow">Why this matchup swings</p><h2>Race-day factors</h2></div></div>
          <div className="stat-grid comparison-kpis">
            <div className="stat-card"><span>Tight battles</span><strong>{comparisonInsights.tightDays} / {comparison.data.shared_days}</strong></div>
            <div className="stat-card"><span>Largest place difference</span><strong>{comparisonInsights.largestPlaceGap || "-"}</strong></div>
            <div className="stat-card"><span>One-pilot skips</span><strong>{comparisonInsights.skippedByPrimary + comparisonInsights.skippedByOpponent}</strong></div>
          </div>
          <p className="chart-note">Tight battles are shared days within 0.5 seconds. Skips count days only one pilot flew; place difference shows the widest gap on a shared day.</p>
        </section>
        <section className="panel">
          <div className="panel-header">
            <div><p className="eyebrow">Daily results</p><h2>{comparisonView === "calendar" ? "Monthly head-to-head calendar" : "Gap to leader chart"}</h2></div>
            <div className="toggle-row">
              <button type="button" className={comparisonView === "calendar" ? "chip active" : "chip"} onClick={() => setComparisonView("calendar")}>Calendar</button>
              <button type="button" className={comparisonView === "gap-chart" ? "chip active" : "chip"} onClick={() => setComparisonView("gap-chart")}>Gap chart</button>
            </div>
          </div>
          {comparisonView === "calendar" ? <>
            <div className="comparison-chart-controls toggle-row"><label><input type="checkbox" checked={showCalendarGaps} onChange={() => setShowCalendarGaps((value) => !value)} /> Show leader gaps</label></div>
              ...
              {months.map(([month, days]) => {
                ...
                return <section className="comparison-month" key={month}>
                  ...
                  <div className="comparison-calendar">
                    ...
                    {days.map((day) => <div className={day.difference_seconds === null ? "comparison-day missing" : "comparison-day"} style={comparisonTileStyle(day)} key={day.date} title={day.date}><Link to={{ pathname: `/${raceClass}`, search: new URLSearchParams({ date: day.date, pilot: primaryPilot, compare_return: comparisonReturnPath }).toString() }} className="comparison-day-link"><small>{day.date.slice(8)}</small><strong>{day.difference_seconds === null ? "-" : `${day.difference_seconds > 0 ? "-" : "+"}${Math.abs(day.difference_seconds)}s`}</strong><em className="calendar-total-times"><span>{primaryPilot}: {formatNumber(day.primary_time, "s")}</span><span>{opponentPilot}: {formatNumber(day.opponent_time, "s")}</span></em>{showCalendarGaps ? <em><span>Leader gap</span><span>{primaryPilot}: {formatNumber(day.primary_gap_to_leader, "s")}</span><span>{opponentPilot}: {formatNumber(day.opponent_gap_to_leader, "s")}</span></em> : null}</Link></div>)}
                  </div>
                </section>;
              })}
            </div>
            ...
            <div className="comparison-chart-controls toggle-row">
              <label><input type="checkbox" checked={showDailyGap} onChange={() => setShowDailyGap((value) => !value)} /> Daily gap (%)</label>
              ...
              <label><input type="checkbox" checked={excludeWorstResult} onChange={() => setExcludeWorstResult((value) => !value)} /> Exclude worst result</label>
            </div>
            <div className="comparison-gap-chart">
              <ResponsiveContainer width="100%" height={380}>
                <LineChart data={gapChartData} margin={{ top: 12, right: 20, left: 0, bottom: 8 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.08)" />
                  <XAxis dataKey="date" minTickGap={32} />
                  <YAxis unit="%" tickFormatter={(value: number) => `${value}%`} />
                  <Tooltip />
                  <Legend />
                  {showDailyGap ? <Line type="monotone" dataKey="primary_daily_gap" name={`${primaryPilot} daily`} stroke="#7ef3c5" dot={false} connectNulls /> : null}
                  {showDailyGap ? <Line type="monotone" dataKey="opponent_daily_gap" name={`${opponentPilot} daily`} stroke="#ff9d5c" dot={false} connectNulls /> : null}
                  {showWeeklyAverage ? <Line type="monotone" dataKey="primary_weekly_gap" name={`${primaryPilot} weekly average`} stroke="#7ef3c5" strokeDasharray="6 4" dot={false} connectNulls /> : null}
                  {showWeeklyAverage ? <Line type="monotone" dataKey="opponent_weekly_gap" name={`${opponentPilot} weekly average`} stroke="#ff9d5c" strokeDasharray="6 4" dot={false} connectNulls /> : null}
                  {showMonthlyAverage ? <Line type="monotone" dataKey="primary_monthly_gap" name={`${primaryPilot} monthly average`} stroke="#7aa7ff" strokeDasharray="2 4" dot={false} connectNulls /> : null}
                  {showMonthlyAverage ? <Line type="monotone" dataKey="opponent_monthly_gap" name={`${opponentPilot} monthly average`} stroke="#ff72b6" strokeDasharray="2 4" dot={false} connectNulls /> : null}
                  <Brush dataKey="date" height={24} stroke="#7ef3c5" travellerWidth={10} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <p className="chart-note">Gap is relative to the daily top-3 average. Daily values are null when that pilot did not fly. Weekly and monthly averages use only each pilot's flown days; when enabled, the worst result is excluded from each rolling window.</p>
          </>}
        </section>
      </> : null}
    </div>
